...
import Component_2.calibration as calibration

from Component_2.calibration import (
...
def test_calibration_by_action_type_rejects_empty_tenant_id(
    tenant_id,
):
    with pytest.raises(ValueError):
...
def test_calibration_by_action_type_rejects_invalid_tenant_id(
    tenant_id,
):
    with pytest.raises(ValueError):
        calibration_by_action_type(tenant_id)
...
def test_worst_predictions_rejects_invalid_tenant_id(
    tenant_id,
):
    with pytest.raises(ValueError):
        worst_predictions(tenant_id)
...
def test_sql_injection_payload_is_rejected(
    ...
    malicious_tenant = "' OR '1'='1' --"

    with pytest.raises(ValueError):
        overall_calibration(malicious_tenant)
...
def test_sql_injection_cannot_turn_tenant_filter_into_global_query(
    ...
    malicious_tenant = f"{tenant_a}' OR 1=1 --"
...
def test_overall_calibration_uses_tenant_scoped_db_context(
    monkeypatch,
    tenant_with_data,
):
    tenant_id = tenant_with_data["tenant_id"]
    ...
    def fake_get_db_context(received_tenant_id):
        nonlocal captured_tenant_id

        captured_tenant_id = received_tenant_id
